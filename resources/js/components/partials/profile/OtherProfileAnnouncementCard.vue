...
export default {
  ...
  methods: {
    ...
    UpdateAnnouncements() {
      ...
      axios.get('/api/accounts/view/announcement/')
        .then(response => {
    ...
    async fetchAnnouncements(initialLoad = false) {
      ...
      this.loading = true;
      try {
        const response = await axios.get(`/api/accounts/view/announcement/${this.userId}?page=${this.currentPage}`);
        if (initialLoad) {
          this.announcements = response.data.announcements || [];
        } else {
          this.announcements = [...this.announcements, ...response.data.announcements];
        }
        this.hasMoreAnnouncement = !!response.data.pagination.next_page_url;
        this.currentPage++;
      } catch (error) {
