<template>
  <div class="card bg-base-200 shadow-md w-full max-w-full border border-base-300 p-4">
    <div class="flex items-start gap-4">
      <!-- Profile Picture -->
      <img :src="profileImage" alt="Profile" class="w-12 h-12 rounded-full" />
      <!-- Input and Buttons -->
      <div class="flex-grow">
        <textarea
...
export default {
  data() {
    return {
      profileImage: 'https://via.placeholder.com/50', // Placeholder, will be updated by fetchProfile method
    };
  },
  created() {
    this.fetchProfile();
  ...
  methods: {
    fetchProfile() {
      axios
        .get('/api/user/profile-picture') // Replace with your actual API endpoint
        .then(response => {
          this.profileImage = response.data.profileImage; // Assuming the response contains 'profileImage'
        })
        .catch(error => {
