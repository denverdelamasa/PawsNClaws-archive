...
export default {
  ...
  data() {
    return {
      combinedFeed: [],
      currentPage: 1, // Single page tracker
      totalPages: 1,
      loading: false,
      noMoreItems: false,
  ...
  methods: {
    async fetchFeed(reset = false) {
      ...
      try {
        ...
        console.log('Fetching combined shelter feed with page:', this.currentPage);

        // Use the new combined feed endpoint
        const response = await axios.get(`/api/feed/shelter/list`, {
          params: {
        ...
        console.log('Received combined feed data:', { feed, pagination });

        // Normalize data for combined feed
        const normalizedFeed = feed.map(item => {
          const data = item.data;
          return {
            id: data.announcement_id || data.post_id || data.event_id,
            type: item.type,
            owner_id: data.shelter_id || data.user_id,
            title: data.title,
            ...
            done_sending_adoption_form: data.done_sending_adoption_form || false,
            is_bookmarked: data.is_bookmarked || false,
            currentSlide: 0, // For event thumbnails
            expanded: false, // For description/caption truncation
          };
        });

        this.combinedFeed = reset ? normalizedFeed : [...this.combinedFeed, ...normalizedFeed];

        // Update pagination
        this.totalPages = pagination.last_page;
        if (normalizedFeed.length > 0 && this.currentPage < this.totalPages) {
          this.currentPage += 1;
        }

        // Check if there are no more items to fetch
        this.noMoreItems = normalizedFeed.length === 0 || this.currentPage > this.totalPages;
