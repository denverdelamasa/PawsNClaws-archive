...
<div v-for="event in events" :key="event.event_id" class="card bg-base-200 w-full shadow-xl my-4 border border-base-300">
        ...
        <div class="flex justify-end w-full">
            <div class="dropdown dropdown-end z-50">
                ...
                <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-300 rounded-box w-40">
                    ...
                    <li v-if="event.shelter_id !== currentUserId"><a href="#">
    ...
    <!-- Thumbnail -->
    <div v-if="event.event_thumbnail && event.event_thumbnail.length > 0" class="m-4 carousel rounded-lg">
        <!-- Dynamically generate carousel items based on event thumbnails -->
        <div 
            v-for="(thumbnail, index) in event.event_thumbnail" :key="index" :id="`slide${index + 1}`" 
            class="carousel-item relative w-full">
            <img :src="thumbnail" :alt="`Event thumbnail ${index + 1}`" class="w-full" />
            <div class="absolute left-5 right-5 top-1/2 flex -translate-y-1/2 transform justify-between">
            <a :href="`#slide${index === 0 ? event.event_thumbnail.length : index}`" class="btn btn-circle">❮</a>
            <a :href="`#slide${index === event.event_thumbnail.length - 1 ? 1 : index + 2}`" class="btn btn-circle">❯</a>
            </div>
        </div>
    </div>
...
export default{
    ...
    methods: {
        ...
        async fetchEvents(reset = false) {
            if (this.loading || (this.noMoreEvents && !reset)) return; // Prevent multiple requests unless resetting
            ...
            try {
                ...
                this.noMoreEvents = true; // No more posts to load
                }
            } catch (error) {
