    ...
    <div class="bg-base-200">
            ...
            <div class="modal-box bg-base-100 text-base-content max-w-3xl w-full">
                ...
                <form @submit.prevent="submitVerificationForm" enctype="multipart/form-data">
                    ...
                    <div class="form-control w-full mt-4">
                        ...
                        <input type="file" @change="handleFileChange" class="file-input file-input-bordered w-full" accept="image/*,.pdf" multiple required>
                    </div>
        ...
        <div v-if="showFormModal" class="fixed inset-0 z-80 flex items-center justify-center bg-black bg-opacity-50">
            <div class="bg-base-300 rounded-lg shadow-lg w-full max-w-4xl max-h-[800px] p-6 relative">
                        ...
                        <div class="form-control mb-4 text-xl">
                            <p>
                                ...
                                <div v-if="selectedApplication.gov_id">
                                    <iframe v-if="selectedApplication.gov_id.endsWith('.pdf')"
                                            :src="`/storage/${selectedApplication.gov_id}`"
                                            width="100%"
                                            height="500px"
                                            class="rounded-lg"
                                            frameborder="0"></iframe>
                                    <img v-else :src="`/storage/${selectedApplication.gov_id}`"
                                         alt="Government ID"
                                         class="rounded-lg" />
                                </div>
                ...
                <button class="absolute top-3 right-3 text-gray-400 hover:text-gray-600" @click.prevent="closeReviewModal">
...
export default {
    ...
    methods: {
        ...
        async fetchVerificationApplications() {
            try {
                const response = await axios.get('/api/verify/applications', {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    },
                });
        ...
        openBioModal() {
            ...
            document.getElementById('bioModal').showModal();
        },
        closeBioModal() {
        ...
        async acceptForm() {
            ...
            try {
                const response = await axios.put(`/api/user/adoption/accept/${applicationId}`, {}, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    },
                });
                ...
                Swal.fire({
                    position: "center",
                    icon: "error",
                    title: "Failed to accept adoption form.",
                    showConfirmButton: false,
                    timer: 3000,
                    timerProgressBar: true,
                    background: "#1e293b",
        ...
        async rejectForm() {
            ...
            try {
                const response = await axios.put(`/api/user/adoption/reject/${applicationId}`, {}, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    },
                });
                ...
                Swal.fire({
                    position: "center",
                    icon: "error",
                    title: "Failed to reject adoption form.",
                    showConfirmButton: false,
                    timer: 3000,
                    timerProgressBar: true,
                    background: "#1e293b",
        ...
        async changeStatusToComplete() {
            ...
            try {
                const response = await axios.put(`/api/user/adoption/complete/${applicationId}`, {}, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    },
                });
                Swal.fire({
                    position: "center",
                    icon: "success",
                    title: "Adoption Process Completed!",
                    showConfirmButton: false,
                    timer: 3000,
                    timerProgressBar: true,
                    background: "#1e293b",
                ...
                Swal.fire({
                    position: "center",
                    icon: "error",
                    title: "Failed to complete adoption process.",
                    showConfirmButton: false,
                    timer: 3000,
                    timerProgressBar: true,
                    background: "#1e293b",
        ...
        async changeStatusToFailed() {
            const applicationId = this.selectedApplication?.application_id;
            if (!applicationId) return;

            try {
                const response = await axios.put(`/api/user/adoption/fail/${applicationId}`, {}, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    },
                });
                ...
                Swal.fire({
                    position: "center",
                    icon: "error",
                    title: "Failed to mark adoption as failed.",
                    showConfirmButton: false,
                    timer: 3000,
                    timerProgressBar: true,
                    background: "#1e293b",
        ...
        handleFileChange(event) {
            ...
            if (this.form.documents.length > 0) {
                ...
                reader.onload = (e) => {
                    ...
                    this.selectedImage = this.form.documents[0];
                    this.$nextTick(() => {
                ...
                reader.readAsDataURL(this.form.documents[0]);
            }
        },
        cancelCrop() {
            this.closeModal();
